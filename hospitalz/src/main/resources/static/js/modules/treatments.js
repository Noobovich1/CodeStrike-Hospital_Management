...
async function loadMasterTreatments(container) {
    ...
    try {
        ...
        if (treatments.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding: 20px;">No treatments found.</td></tr>';
            select.innerHTML = '<option value="">No treatments available</option>';
            return;
        }

        tbody.innerHTML = treatments.map(t => {
            const statusColor = t.isActive ? 'var(--status-success)' : 'var(--text-secondary)';
            return `
                <tr style="border-bottom: 1px solid var(--border-color);">
                    ...
                    <td style="padding: 12px;">
                        <div class="action-group">
                            ...
                            <button class="btn-icon btn-deactivate-treatment" data-id="${t.treatmentId}" title="Deactivate/Delete Treatment">
        ...
        select.innerHTML = activeTreatments.map(t => 
        ...
        container.querySelectorAll('.btn-deactivate-treatment').forEach(btn => {
            btn.onclick = async () => {
                const treatmentId = btn.dataset.id;
                if (confirm(`Are you sure you want to deactivate treatment ID ${treatmentId}?`)) {
                    try {
                        await api.delete(`/treatments/${treatmentId}`);
                        alert('Treatment deactivated successfully!');
                        loadMasterTreatments(container);
                    } catch (error) {
                        alert('Error deactivating treatment: ' + error.message);
                    }
                }
            };
        });
        ...
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; color: var(--status-danger); padding: 20px;">Error loading treatments</td></tr>';
    }
}
...
function setupTreatmentsEvents(container) {
    ...
    btnCreate.addEventListener('click', () => {
        document.getElementById('treatment-form-title').textContent = "Create Treatment";
    ...
    searchBtn.addEventListener('click', async () => {
        ...
        prescHistoryBody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 20px;">Searching...</td></tr>';
        ...
        try {
            ...
            if (!records || records.length === 0) {
                prescHistoryBody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 20px;">No prescription records found.</td></tr>';
                return;
            }

            prescHistoryBody.innerHTML = records.map(r => `
                <tr style="border-bottom: 1px solid var(--border-color);">
